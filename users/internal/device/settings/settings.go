...
func (r *repository) ModifyDeviceSettings(ctx context.Context, ds *DeviceSettings) error {
	...
	*ds = *resp[0]
	...
	return errors.Wrapf(r.sendDeviceSettingsMessage(ctx, ds), "failed to send device settings message: %#v", ds)
}

func (r *repository) insert(ds *DeviceSettings) error {
	if ds.Language == "" {
		ds.Language = "en"
	}
...
func (r *repository) sendDeviceSettingsMessage(ctx context.Context, ds *DeviceSettings) error {
